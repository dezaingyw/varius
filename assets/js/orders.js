// assets/js/orders.js (MODIFIED - WhatsApp integration + minor role checks)
// Added: WhatsApp action button (whatsapp-btn). When clicked by motorizado (or admin),
// updates order document with communicationStatus = 'contacting' and opens WhatsApp.
//
// Note: This file is the same as your existing one with additions in renderOrders() for the WhatsApp button
// and the new function openWhatsApp(orderId).
...
/* (rest of helper functions and state are unchanged) */
...
function renderOrders() {
    ...
    visible.forEach(o => {
        ...
        const btnView = `<button class="icon-btn view-btn" data-order="${o.id}">...</button>`;
        ...
        // WhatsApp button: available to motorizado and administrador and sellers too if desired.
        // We enable platform-wide but disallow if no phone is present. We'll include a data-phone attribute build later via event listener.
        const phone = (o.customerData && (o.customerData.phone || o.customerData.telefono || o.customerData.mobile)) || '';
        const whatsappBtn = `<button class="icon-btn whatsapp-btn" data-order="${o.id}" ${phone ? '' : 'disabled'} title="${phone ? 'Abrir WhatsApp' : 'No hay teléfono disponible'}">WhatsApp</button>`;

        const actionsTd = `<td class="actions">${assignBtn} ${btnView} ${btnMarkPaid} ${btnMarkSent} ${btnMarkDelivered} ${whatsappBtn}</td>`;
...
async function openWhatsApp(orderId) {
    try {
        ...
        // fetch latest order to ensure phone
